# AuraBlock
### Device-Wide DNS AdBlocker for Linux
...
2. **Fast Engine**: Written in Go with O(1) domain lookups, suffix-matching subdomain resolution, and an in-memory TTL caching layer.
3. **Stunning Web Dashboard**: An dark-themed dashboard featuring real-time analytical charts, stats, and dynamic settings.
4. **Real-time Query Stream**: Streams DNS requests live to your dashboard using Server-Sent Events (SSE) for system monitoring.
