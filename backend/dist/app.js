...
function populateDomainList(listId, items, isBlockedList) {
  ...
  items.forEach(item => {
    ...
    li.innerHTML = `
      ...
      <span class="domain-count" style="${isBlockedList ? 'color:#0056b3;' : ''}">${item.count}</span>
    `;
    list.appendChild(li);
  });
}
  ...
  fetch(`${API_BASE}/api/history?status=BLOCKED&limit=200`)
    ...
    .then(logs => {
      ...
      filtered.forEach(log => {
        ...
        tr.innerHTML = `
          <td class="col-time">${timeStr}</td>
          <td class="col-domain" style="color:#0056b3; font-family:'JetBrains Mono', monospace;" title="${log.domain}">${log.domain}</td>
          <td><span class="col-type">${log.query_type}</span></td>
          <td class="col-client">${log.client_ip}</td>
